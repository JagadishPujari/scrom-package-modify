/*1.4.6*/ function DopPlayer() {
  ...
  function n() {
    function t() {
      var e =
        (y.fullScreenElement && null !== y.fullScreenElement) ||
        y.mozFullScreen ||
        y.webkitIsFullScreen;
      if (e) {
    ...
    if (
      ...
      a &&
        ...
        ((c.orc = 0),
        (c.onorientationchange = function () {
          1 === s && (this.orc = 1);
        }),
        top === self))
    ) {
    ...
    (o.backgroundColor = "#ECECEC"),
      (o.borderTop = "1px solid #DBDADA"),
      "undefined" != typeof r
        ? (y.getElementsByTagName("body")[0].style.backgroundColor = r)
        : (y.getElementsByTagName("body")[0].style.backgroundColor =
            d.BackColr);
    var g = p("resizer");
    ...
    v.setAttribute("style", "font-weight: bold; color: #525151;"),
      (v.id = "times"),
      m.appendChild(v);
    ...
    10 > L && (L = "0" + L),
      ...
      10 > b && (b = "0" + b),
      0 === b
        ? (m.innerHTML = m.innerHTML + ":" + x + ":" + L)
        : (m.innerHTML = m.innerHTML + ":" + b + ":" + x + ":" + L),
      (p("buttonDiv").style.display = "block"),
      (progressContainer = p("progressContainer")),
      (progressContainer.style.display = "block"),
      (p("time").style.display = "block");
    var w = p("status");
    (w.style.display = "block"),
      d.ShowNotes && (p("notesButton").style.display = "block"),
      (p("statusTotal").value = "/ " + d.TotalSlides);
    var M =
    ...
    S
      ? ((C.style.backgroundImage = M), (C.onclick = t))
      : ((m.style.right = "78px"),
        (progressContainer.style.right = "80px"),
        (w.style.right = "7px"),
        C.parentNode.removeChild(C)),
      (p("statusCurrent").onkeydown = i),
      (p("playN").onclick = function () {
        h && (l.ClosePlayButton(), l.ForceStartSlidePlay(), (h = !1)), l.Next();
        // console.log(_.uniq(document.links))
        for (var i = document.links.length; i-- > 0;){
            // if (document.links[i].hostname === 'client.stackroute.in')
            // {
            //     let newUrl = 'https://' + localStorage.getItem("userOrg") + '.stackroute.in'
            ...
            {
                let newUrl = 'https://' + (document.links[i].hostname.replace("client", localStorage.getItem("userOrg")));
                document.links[i].innerHTML = newUrl;
                document.links[i].href =  newUrl;
            }
        }
    ...
    (H.onclick = function () {
      ...
      (H.onmouseover = function () {
        var e = this.style;
        u
          ? (e.backgroundPosition = "-148px 0px")
          : (e.backgroundPosition = "-50px 0px");
      }),
      (H.onmouseout = function () {
        var e = this.style;
        u
          ? (e.backgroundPosition = "-99px 0px")
          : (e.backgroundPosition = "-1px 0px");
      });
  }
  function i(e) {
  ...
  Pcl = function () {
    function e() {
      a &&
        ...
        ((c.iabho = y.createElement("div")),
        y.body.appendChild(c.iabho),
        (c.iabho.style.height = "1000px"),
        setTimeout(function () {
    ...
    (this.r = function () {
      ...
      var n,
        ...
        g = 0;
      d.MaxScale > 0
        ? ((n = y > d.Height * d.MaxScale ? d.Height * d.MaxScale : y),
          (i = r > d.Width * d.MaxScale ? d.Width * d.MaxScale : r))
        : ((n = y), (i = r)),
        "block" === p("panel").style.display && (g = 250),
        (o =
          i - g < d.Width || n - h < d.Height
            ? (d.Width - (i - g)) / d.Width < (d.Height - (n - h)) / d.Height
              ? 1 - (d.Height - (n - h)) / d.Height
              : 1 - (d.Width - (i - g)) / d.Width
            : (i - g - d.Width) / d.Width > (n - h - d.Height) / d.Height
            ? 1 + (n - h - d.Height) / d.Height
            : 1 + (i - g - d.Width) / d.Width),
        l.SetScale(o),
        0 === d.MaxScale && (i = g + d.Width * o),
        (u.style.height = t(n) + "px"),
        (u.style.width = t(i) + "px"),
        (u.style.top = t((y - n) / 2) + "px"),
        (u.style.left = t((r - i) / 2) + "px");
      var f = p("resizer");
      (f.style.top = 0),
        (f.style.left = 0),
        (f.style.webkitTransform = "scale(" + t(o) + ")"),
        (f.style.MozTransform = "scale(" + t(o) + ")"),
        (f.style.OTransform = "scale(" + t(o) + ")"),
        (f.style.msTransform = "scale(" + t(o) + ")");
      var m = p("contentHolder");
      (m.style.height = t(d.Height * o) + "px"),
        (m.style.width = t(d.Width * o) + "px");
      var v = t(g + (i - g - o * d.Width) / 2),
        L = t((n - h - o * d.Height) / 2);
      (m.style.marginLeft = v + "px"), (m.style.marginTop = L + "px");
      var x = p("loadingImg");
      (x.style.height = t(d.Height * o) + "px"),
        (x.style.width = t(d.Width * o) + "px"),
        (x.style.marginTop = L + "px"),
        (x.style.marginLeft = v + "px");
      var b = p("panel");
      (b.style.height = t(n - h) + "px"),
        (p("noteDisplay").style.height = t(n - h - 40) + "px"),
        (p("buttonDiv").style.left = t(i / 2 - 25) + "px");
      var w = (i / 2 - 70 - 95) / 200;
      (w = w > 0.8 ? 0.8 : w), (w = 0.33 > w ? 0.33 : w);
      var M = p("progress");
      (M.style.webkitTransform = "scale(" + t(w) + ",1)"),
        (M.style.MozTransform = "scale(" + t(w) + ",1)"),
        (M.style.OTransform = "scale(" + t(w) + ",1)"),
        (M.style.msTransform = "scale(" + t(w) + ",1)"),
        (M.parentNode.style.width = t(200 * w) + "px");
      ...
      i > 700
        ? ((P.opacity = 1),
          ...
          (S.display = "block"),
          (M.style.opacity = 1),
          null !== b && ((T.right = "65px"), (T.opacity = 1)))
        : ((P.opacity = 0),
          (S.display = "none"),
          (M.style.opacity = 0),
          (C.opacity = 1),
          null !== b && (T.opacity = 1),
          370 > i && (null !== b && (T.opacity = 0), a || (C.opacity = 0)));
      var H = p("play");
      if (null !== H) {
        var D = H.style;
      ...
      var I = p("slideNote"),
        k = 10;
      (I.style.height = parseInt(b.style.height) - k + "px"),
        (p("noteDisplay").style.height =
          parseInt(b.style.height) - k - 55 + "px");
    }),
      ...
      (this.n = function (e) {
        ...
        for (var i = t.length, o = 0; i > o; o++) {
          ...
          for (var d = r.length, u = 0; d > u; u++) {
            ...
            (f.innerHTML = g),
              h.bold && (f.style.fontWeight = "bold"),
              h.italic && (f.style.fontStyle = "italic"),
              h.underline && h.line_through
                ? (f.style.textDecoration = "line-through underline")
                : h.line_through
                ? (f.style.textDecoration = "line-through")
                : h.underline && (f.style.textDecoration = "underline"),
              c.appendChild(f);
          }
          n.appendChild(c);
        }
  ...
  var f = new Pcl();
  if (
    ((p("notesButton").onclick = function () {
      var e = p("panel");
    ...
    return (
      (m.innerHTML = ""),
      (m.style.width = "0px"),
      void (y.body.innerHTML =
        "<h2>Oops! Your browser does not support HTML5. You need to upgrade your browser to view this content.</h2>")
    );
  }
  var v = o("step"),
    L = o("slide");
